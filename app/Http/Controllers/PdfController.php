...
class PdfController extends Controller
{
  ...
  */
  ...
  public function scout_advancement($id) {
    if(MeritBadgeStarted::where('scout_id', $id)->get() != "[]") { // checks to see if there is a merit badge started by this scout
      $meritBs_s = MeritBadgeStarted::where('scout_id', $id)->get(); // gets all the merit badges started
      $reqs_s = NULL;
      $meritBs = NULL;
      $scout = Scout::find($id); // finds the scout
      foreach($meritBs_s as $key=>$meritB_s) {
        $reqs_s[] = Requirement_started::where('meritB_id', $meritB_s->id)->get(); // gets all the requirements started from the merit badges started
        $meritBs[] = MeritBadge::find($meritB_s->meritBadge_id); // finds all the merit badges
      }
