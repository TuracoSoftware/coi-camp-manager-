...
class TroopController extends Controller
{
  ...
  public function index() {
    ...
    if(Auth::user()->type == 'admin' || Auth::user()->type == 'director' || Auth::user()->type == 'staff')
      ...
      $troops = Troop::where('user_id', Auth::id())->get();
    ...
    if(Auth::user()->type == 'admin' || Auth::user()->type == 'director' || Auth::user()->type == 'staff') {
      ...
      if(Auth::user()->troop)
		    $scout = Scout::where('troop_id', Auth::user()->troop->id)->get();
		  else
		    $scout = [];
  ...
  public function addScoutUpdate($id, Request $request) {
    ...
    public function edit($id) {
      $troop = Troop::find($id);
      if($troop)
        if($troop->user == Auth::user() || Auth::user()->type == 'admin' || Auth::user()->type == 'director')
